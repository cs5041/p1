...
function onloadHandler() {
    ...
    g_backgroundImg.onload = function () {
        ...
        canvasElement.classList.toggle('hidden', !('serial' in navigator));
...
(function () {
    Asteroids.AttractorScene = function (game) {
        ...
        const serialInit = async (game) => {
            if (!game.reader) {
                await game.port.open({baudRate: 115200});
                const textEncoder = new TextEncoderStream();
                const textDecoder = new TextDecoderStream();
                const writableStreamClosed = textEncoder.readable.pipeTo(game.port.writable);
                const readableStreamClosed = game.port.readable.pipeTo(textDecoder.writable);
                game.reader = textDecoder.readable
                    .pipeThrough(new TransformStream(new LineBreakTransformer()))
                    .getReader();
                game.writer = textEncoder.writable
                    .getWriter();
            }
        }
        var fMouseDown = function (e) {
            if (e.button === 0 && me.imagesLoaded) {
                ...
                } else if (!me.port && navigator?.serial) {
                    navigator?.serial?.requestPort({filters: [{usbVendorId: 0x0d28}]}).then((port) => {
                        game.port = port;
                        serialInit(game);
                        me.start = true;
                        game.audioContext?.resume();
    ...
    extend(Asteroids.GameScene, Game.Scene,
        {
                ...
                serialRead(this.game.port, this.game.reader, (value) => {
                    ...
                    if (command.length === 2) {
                        switch (command[0].toLowerCase()) {
                            ...
                            case 'e': //teleport
                                if (command[1] === '1') {
                                    this.player.position.x = Math.floor(GameHandler.width * Math.random());
                                    this.player.position.y = Math.floor(GameHandler.height * Math.random());
                                }
                                break;
            ...
            onBeforeRenderScene: function onBeforeRenderScene() {
                ...
                if (typeof this.input.thrust === 'number') {
                    this.player.thrust(this.input.thrust);
                } else if (this.input.thrust) {
                    this.player.thrust();
                }
                // } else if (iOS) {
